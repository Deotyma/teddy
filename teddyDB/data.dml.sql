...
INSERT INTO annonces (title, text_annonce, email, sharing_method_name, category_name, date) VALUES
    ('Old book for sale', 'Montessori book for sale.', 'john.doe@example.com', 'sell', 'book', '2023-11-02'),
    ('Kids toy', 'Puzzle of 3 elements to give.', 'mdupont@example.com', 'give', 'toy', '2023-11-03');




INSERT INTO favorites(id, email) 
VALUES ((SELECT a.id FROM annonces a WHERE a.title = 'Old book for sale'),
        (SELECT u.email FROM users u WHERE u.nick_name = 'mdupont')),
       ((SELECT a.id FROM annonces a WHERE a.title = 'Kids toy'),
